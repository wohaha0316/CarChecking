...
object SpecBucketer {
    ...
    data class Detail(
        ...
        val lenMm: Int?,
        val widthMm: Int?,
        val source: Char // 'O' override, 'T' text, 'M' master, '-' none
    )
    ...
    fun parseLenWidthFromText(carInfo: String): Pair<Int?, Int?> {
        ...
        numRegex.findAll(txt).forEach { m ->
            ...
            val windowEnd = min(tokens.length, spanStart + 12)
            val ctx = tokens.substring(windowStart, windowEnd)
    ...
    /** 20/15/10 cm 간격으로 5.0 이상 ~ 4.0 미만 범위를 생성 */
    ...
    /**
     * 상세행 구성
     * 우선순위: Override > Text(carInfo) > Master > 없음
     */
    fun buildDetails(
        keyId: String,
        rows: List<CheckRow>,
        ...
        rows.forEachIndexed { idx, r ->
            ...
            val master = SpecMaster.findForRow(r)

            val len = ov?.lenMm ?: text.first ?: master?.lengthMm
            val wid = ov?.widthMm ?: text.second ?: master?.widthMm
    ...
    /**
     * 매칭 실패(정보없음) 목록
     * 나중에 CRUD 추가 대상 뽑을 때 이걸 쓰면 된다.
     */
    fun unknownDetails(details: List<Detail>): List<Detail> {
        return details.filter { it.lenMm == null || it.widthMm == null || it.source == '-' }
    }

    fun unknownCount(details: List<Detail>): Int {
        return unknownDetails(details).size
    }
